...
const UploadTemplateDialog = ({ onSave }: { onSave: (name: string, uri: string) => void }) => {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Upload New W4 Template</DialogTitle>
                    <DialogDescription>Provide a name for the template and upload the document.</DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4">
                    <div className="space-y-2">
                        <Label htmlFor="template-name">Template Name</Label>
                        <Input id="template-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., 2024 Federal W-4" />
                    </div>
                     <div className="space-y-2">
...
export default function W4Page() {
  ...
  const handleSaveTemplate = (name: string, documentUri: string) => {
    ...
    toast({ title: 'Template Uploaded', description: `Template "${name}" has been saved.` });
  };
  ...
  const handleRename = () => {
    if (templateToRename && newName) {
        ...
        toast({ title: 'Template Renamed', description: 'The template name has been updated.' });
  ...
  const handleDelete = () => {
    if(templateToDelete) {
        ...
        toast({variant: 'destructive', title: 'Template Deleted', description: `Template "${templateToDelete.name}" has been moved to the trash.`});
            ...
            <TabsContent value="templates">
                <Card>
                    <CardHeader className="flex-row items-start justify-between">
                        <div>
                            <CardTitle className="font-headline">W4 Template Library</CardTitle>
                            <CardDescription>
                                Manage, preview, and send your W4 templates.
                            </CardDescription>
                        </div>
