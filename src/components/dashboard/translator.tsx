...
import { ArrowRightLeft, Volume2, Mic, Copy, Mail } from 'lucide-react';
...
export function Translator({ onCopyToNote }: TranslatorProps) {
    ...
    const handleTranslate = useCallback(async (text: string, source: string, target: string, setTextCallback: (text: string) => void, speakResult: boolean = false) => {
        if (!text.trim()) {
            return;
        }
        setIsLoading(true);
    ...
    const handleEmail = () => {
        const subject = `Translation: ${inputText.substring(0, 20)}...`;
        const body = `Original (${languages.find(l=>l.code === sourceLang)?.name}):\n${inputText}\n\nTranslated (${languages.find(l=>l.code === targetLang)?.name}):\n${outputText}`;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2 flex-wrap">
                        ...
                        {onCopyToNote && (
                            <Button variant="secondary" size="sm" onClick={() => onCopyToNote(outputText)} disabled={!outputText}>
                                <Copy className="mr-2" />
                                Copy to Note
                            </Button>
                        )}
                        <Button variant="secondary" size="sm" onClick={handleEmail} disabled={!outputText}>
                            <Mail className="mr-2" />
                            Email Translation
                        </Button>
                    </div>
