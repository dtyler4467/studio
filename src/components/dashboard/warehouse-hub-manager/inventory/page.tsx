...
import { FileDown, FileText, MoreHorizontal, PlusCircle, AlertCircle, ShoppingCart, FilePlus, Users2, Boxes } from 'lucide-react';
import { useSchedule } from '@/hooks/use-schedule';

export default function InventoryPage() {
    const { inventoryItems } = useSchedule();

    const totalValue = inventoryItems.reduce((acc, item) => acc + (item.qty * (item.price || 0)), 0);
    const lowStockItems = inventoryItems.filter(item => item.qty <= item.reorderPoint).length;
    const totalItems = inventoryItems.reduce((acc, item) => acc + item.qty, 0);
